...
namespace apsi {
    ...
    */
    ...
    using HashRequest = std::unique_ptr<network::SenderOperationHash>;
    ...
    */
    ...
    // hash start
    inline HashRequest to_hash_request(Request &request)
    {
        if (request == nullptr || request->type() != apsi::network::SenderOperationType::sop_hash)
            return nullptr;
        return HashRequest(static_cast<apsi::network::SenderOperationHash *>(request.release()));
    }
