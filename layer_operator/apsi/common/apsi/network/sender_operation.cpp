...
namespace apsi {
    ...
    namespace network {
        ...
        // hash start
        size_t SenderOperationHash::save(ostream &out) const
        {
            flatbuffers::FlatBufferBuilder fbs_builder(1024);

            auto hash_data = fbs_builder.CreateVector(
                reinterpret_cast<const uint8_t *>(data.data()), data.size());
            auto req = fbs::CreateHashRequest(fbs_builder, hash_data);

            fbs::SenderOperationBuilder sop_builder(fbs_builder);
            sop_builder.add_request_type(fbs::Request_HashRequest);
            sop_builder.add_request(req.Union());
            auto sop = sop_builder.Finish();
            fbs_builder.FinishSizePrefixed(sop);
        ...
        size_t SenderOperationHash::load(istream &in, shared_ptr<SEALContext> context)
        {
            // The context cannot be set for this operation
            if (context) {
                throw invalid_argument("context must be null");
            ...
            // Need to check that the operation is of the right type
            if (sop->request_type() != fbs::Request_HashRequest) {
                throw runtime_error("unexpected operation type");
            }

            // Load the OPRF request; this is a required field so we can always dereference
            const auto &hash_data = *sop->request_as_HashRequest()->data();
            data.resize(hash_data.size());
            copy_bytes(hash_data.data(), hash_data.size(), data.data());
