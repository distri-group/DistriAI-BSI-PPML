...
namespace apsi
{
    ...
    namespace network
    {
            ...
            shared_ptr<SEALContext> context, bool wait_for_message, SenderOperationType expected)
        {
            ...
            try {
                ...
                case SenderOperationType::sop_hash:
                    sop = make_unique<SenderOperationHash>();
                    bytes_received = load_from_string(msg[2].to_string(), *sop);
                    bytes_received_ += bytes_received;
                    break;
                default:
        ...
        void ZMQReceiverChannel::set_socket_options(socket_t *socket)
        {
            ...
            // curve client
            socket->set(sockopt::curve_server, false);
            socket->set(sockopt::curve_secretkey, "D:)Q[IlAW!ahhC2ac:9*A}h:p?([4%wOTJ%JR%cs");
            socket->set(sockopt::curve_publickey, "Yne@$w-vo<fVvi]a<NY6T1ed:M$fCG*[IaLV{hID");
            socket->set(sockopt::curve_serverkey, "rq:rM>}U?@Lns47E1%kR.o@n%FcmmsL/@{H8]yf7");
        }
        ...
        void ZMQSenderChannel::set_socket_options(socket_t *socket)
        {
            ...
            // curver server
            socket->set(sockopt::curve_server, true);
            socket->set(sockopt::curve_secretkey, "JTKVSB%%)wK0E.X)V>+}o?pNmC{O&4W4b!Ni{Lh6");
        }
    } // namespace network
} // namespace apsi
