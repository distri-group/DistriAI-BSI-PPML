...
namespace apsi {
    ...
    namespace sender {
        ...
        void ZMQSenderDispatcher::runPhase1(const atomic<bool> &stop, int port, ZMQSenderChannel &chl)
        {
            stringstream ss;
            ss << "tcp://*:" << port;
            ...
            while (!stop) {
                ...
                case SenderOperationType::sop_parms:
                    cout << "sender<received parameter request>: \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
                    APSI_LOG_INFO("Received parameter request");
                    dispatch_parms(move(sop), chl);
                    cout << "sender<response parameter request>: \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
                    break;

                case SenderOperationType::sop_oprf:
                    cout << "sender<received OPRF request>:      \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
                    APSI_LOG_INFO("Received OPRF request");
                    dispatch_oprf(move(sop), chl);
                    cout << "sender<response OPRF request>:      \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
                    break;
                case SenderOperationType::sop_hash:
                    dispatch_hash(move(sop));
                    return;
                default:
        ...
        void ZMQSenderDispatcher::runPhase2(const atomic<bool> &stop, int port, ZMQSenderChannel &chl)
        {
            auto seal_context = sender_db_->get_seal_context();
            ...
            while (!stop) {
                ...
                case SenderOperationType::sop_query:
                    cout << "sender<received query request>:      \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;

                    APSI_LOG_INFO("Received query");
                    dispatch_query(move(sop), chl);
                    cout << "sender<response query request>:      \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
                    return;
                    break;
        ...
        // hash start
        void ZMQSenderDispatcher::dispatch_hash(unique_ptr<ZMQSenderOperation> sop)
        {

            HashRequest hash_request = to_hash_request(move(sop->sop));
            auto v = hash_request->data;
            auto seed_increase = (v[2] << 16) | (v[1] << 8) | v[0];
            seed_increase_ = seed_increase;
        }
        // hash end
