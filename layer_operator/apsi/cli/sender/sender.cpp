...
time_point<high_resolution_clock> time_pre_start;
time_point<high_resolution_clock> time_pre_end;
time_point<high_resolution_clock> time_query_start;
time_point<high_resolution_clock> time_query_end;

time_point<system_clock> time_sender_pre_start;
time_point<system_clock> time_sender_pre_end;
time_point<system_clock> time_sender_query_start;
...
shared_ptr<SenderDB> create_sender_db(
    ...
    bool compress,
    network::ZMQSenderChannel &ch,
    int port);

int main(int argc, char *argv[])
{
    cout << "############# sender system begin ###############" << endl;
...
shared_ptr<SenderDB> try_load_csv_db(const CLP &cmd, OPRFKey &oprf_key, network::ZMQSenderChannel &chl)
{
    ...
    cout << "sender<load db begin>:            \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
    ...
    cout << "sender<load db end>:              \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;

    // nonce_byte_cout = 16, compress =0
    return create_sender_db(
        *db_data, move(params), oprf_key, cmd.nonce_byte_count(), cmd.compress(), chl, cmd.net_port());
}
...
int start_sender(const CLP &cmd)
{
    // time_sender_pre_start = high_resolution_clock::now();
    time_sender_pre_start = system_clock::now();
    ...
    OPRFKey oprf_key;
    network::ZMQSenderChannel chl;
    if (!(sender_db = try_load_sender_db(cmd, oprf_key)) &&
        !(sender_db = try_load_csv_db(cmd, oprf_key, chl))) {
    ...
    cout << "sender<construct polynomials end>: \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
    ...
    dispatcher.runPhase2(stop, cmd.net_port(), chl);
    // time_query_end = high_resolution_clock::now();

    // cout << "sender pre-start: " << "\t" << duration_cast<milliseconds>(time_sender_pre_start) << "ms" << endl;
    // cout << "sender pre-end: " << "\t" << duration_cast<milliseconds>(time_sender_pre_end) << "ms" << endl;
    // cout << "sender query-start: " << "\t" << duration_cast<milliseconds>(time_sender_query_start) << "ms" << endl;

    // APSI_LOG_INFO("pre time: " << "\t" << duration_cast<milliseconds>(time_pre_end-time_pre_start).count() << "ms");
    // APSI_LOG_INFO("query time: " << "\t" << duration_cast<milliseconds>(time_query_end-time_query_start).count() << "ms");
    cout << "############# sender system end   ###############" << endl;
    ...
    int port)
{
    ...
    if (holds_alternative<CSVReader::UnlabeledData>(db_data)) {
        ...
        try {
            ...
            atomic<bool> stop = false;
            ZMQSenderDispatcher dispatcher(sender_db, sender_db->get_oprf_key());
            // The dispatcher will run until stopped.

            dispatcher.runPhase1(stop, port, chl);
            sender_db->set_seed_increase(dispatcher.get_seed_increase());
