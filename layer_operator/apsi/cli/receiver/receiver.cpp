...
time_point<system_clock> time_recive_query_end;

int main(int argc, char *argv[])
{
    cout << "############# receiver system begin #############" << endl;
...
int remote_query(const CLP &cmd)
{
    cout << "receiver<connect begin>:        \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
    ...
    cout << "receiver<connect end>:          \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
    cout << "receiver<request params begin>: \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
    ...
    cout << "receiver<request params end>:   \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;

    // cout << "receiver<load db begin>:      \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
    ...
    // cout << "receiver<load db end>:        \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
    ...
    cout << "receiver<request OPRF begin>     \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
    try {
    ...
    cout << "receiver<request OPRF end>       \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;

    // cout << "receiver query request: \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
    vector<MatchRecord> query_result;
    try {
        // APSI_LOG_INFO("Sending APSI query");
        query_result = receiver.request_query(oprf_items, label_keys, channel, cmd.threads());
        // APSI_LOG_INFO("Received APSI query response");
    } catch (const exception &ex) {
        APSI_LOG_WARNING("Failed sending APSI query: " << ex.what());
        return -1;
    }
    cout << "receiver<results process begin>: \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;

    print_intersection_results(orig_items, items_vec, query_result, cmd.output_file());
    cout << "receiver<results process end>:   \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;

    // print_transmitted_data(channel);
    // print_timing_report(recv_stopwatch);

    cout << "############# receiver system end   #############" << endl;
    return 0;
}
