...
namespace apsi
{
    ...
    namespace receiver
    {
        ...
        unique_ptr<SenderOperation> Receiver::CreateHashRequest(vector<uint8_t> data)
        {
            auto sop = make_unique<SenderOperationHash>();
            sop->data = data;

            return sop;
        }
        void Receiver::SendHashToServer(vector<uint8_t> data, NetworkChannel &chl)
        {
            chl.send(CreateHashRequest(data));
        }
        ...
        std::pair<std::shared_ptr<kuku::KukuTable>, int> Receiver::threadCuckoo(int table_size,
                                                                                int hash_func_count,
                                                                                int attempts,
                                                                                ...
                                                                                int chunk)
        {
            ...
            //     0,                                      // Not using a stash
            //     hash_func_count, // Number of hash functions
            //     { 0, 0 },                               // Hardcoded { 0, 0 } as the seed
            //     attempts,           // The number of insertion attempts
            //     { 0, 0 });                              // The empty element can be set to anything
            using ints = std::array<unsigned char, 16>;
            int no = -1;
            std::shared_ptr<kuku::KukuTable> cuckoo;
            vector<kuku::item_type> new_items(items.size());
            for (size_t item_idx = 0; item_idx < items.size(); item_idx++) {
                const auto &item = items[item_idx];
                new_items[item_idx] = item.get_as<kuku::item_type>().front();
                // for (int j = 0; j < tdnum*chunk; j++) {
                //     *reinterpret_cast<std::uint64_t*>(new_items[item_idx].data()) += 1;
                //     *reinterpret_cast<std::uint64_t*>(new_items[item_idx].data() + 8) += !*reinterpret_cast<std::uint64_t*>(new_items[item_idx].data()) ? 1 : 0;
                // }
                //*reinterpret_cast<__uint128_t*>(new_items[item_idx].data()) += tdnum*chunk;
            }
            vector<bool> first_flag(items.size());
            for (int i = 0; i < first_flag.size(); i++) {
                first_flag[i] = true;
            }
            for (int i = tdnum * chunk; i < (tdnum + 1) * chunk; i++) {
                cuckoo = make_shared<KukuTable>(
                    table_size,      // Size of the hash table
                    0,               // Not using a stash
                    hash_func_count, // Number of hash functions
                    ints{0, 0},      // Hardcoded { 0, 0 } as the seed
                    attempts,        // The number of insertion attempts
                    ints{0, 0});
                ...
                for (size_t item_idx = 0; item_idx < items.size(); item_idx++) {
                    // if (first_flag[item_idx]) {
                    //     first_flag[item_idx] = false;
                    // } else {
                    //     *reinterpret_cast<std::uint64_t*>(new_items[item_idx].data()) += 1;
                    //     *reinterpret_cast<std::uint64_t*>(new_items[item_idx].data() + 8) += !*reinterpret_cast<std::uint64_t*>(new_items[item_idx].data()) ? 1 : 0;
                    // }
                    const auto &item = items[item_idx];
                    auto new_item = item.get_as<kuku::item_type>().front();
                    *reinterpret_cast<__uint128_t *>(new_item.data()) += i;
                    if (!cuckoo->insert(new_item)) {
        ...
        pair<Request, IndexTranslationTable> Receiver::create_query(const vector<HashedItem> &items, int tds, network::NetworkChannel &chl)
        {
            cout << "receiver<Collision-Free Hash begin>:\t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
            ...
            auto attempts = cuckoo_table_insert_attempts;
            if (params_.table_params().table_size == 1) {
                attempts = 1;
            } else {
            }

            // auto chunk = 16777215/tds; // 0xFFFFFF
            // auto chunk = 4294967295/tds;  // 0xFFFFFFFF
            auto chunk = 1048575 / tds; // 0xFFFFF
            std::vector<std::future<std::pair<std::shared_ptr<kuku::KukuTable>, int>>> fut(tds);
            for (int t = 0; t < tds; t++) {
                fut[t] = std::async(std::launch::async, [&] { return Receiver::threadCuckoo(
                                                                  params_.table_params().table_size,
                                                                  params_.table_params().hash_func_count,
                                                                  attempts,
                                                                  items, t - 1, chunk); });
            }
            ...
            while (flag) {
                for (int t = 0; t < tds; t++) {
                    if (fut[t].wait_for(span) == std::future_status::ready) {
                        auto ret = fut[t].get();
                        cuckoo = std::get<0>(ret);
                        no = std::get<1>(ret);
                        tdnum = t;
                        flag = false;
                        break;
            ...
            vector<uint8_t> seed_increase = {((uint32_t)no & (uint8_t)0xFF), (((uint32_t)no >> 8) & (uint8_t)0xFF), (((uint32_t)no >> 16) & (uint8_t)0xFF)};
            Receiver::SendHashToServer(seed_increase, chl);
            cout << "receiver<Collision-Free Hash end>:\t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
            ...
            */
            ...
            // cout << "get: " << tdnum << ", " << no << endl;
            cout << "receiver<create query begin>:      \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
            vector<kuku::item_type> new_item(items.size());
            for (size_t item_idx = 0; item_idx < items.size(); item_idx++) {
                new_item[item_idx] = items[item_idx].get_as<kuku::item_type>().front();
                *reinterpret_cast<__uint128_t *>(new_item[item_idx].data()) += no;
            }
            auto table = cuckoo->table();
            for (size_t item_idx = 0; item_idx < new_item.size(); item_idx++) {
                auto item_loc = cuckoo->query(new_item[item_idx]);
                itt.table_idx_to_item_idx_[item_loc.location()] = item_idx;
                table[item_loc.location()] = items[item_idx].get_as<kuku::item_type>().front();
            }
            ...
            // prepare_data
            {
                ...
                // bundle_idx_count = 1, items_per_bundle = 512, item_bit_count = 120
                for (uint32_t bundle_idx = 0; bundle_idx < params_.bundle_idx_count();
                    ...
                    gsl::span<const item_type> bundle_items(
                        table.data() + bundle_idx * params_.items_per_bundle(),
                        params_.items_per_bundle());
            ...
            cout << "receiver<create query end>:      \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
            ...
            int tds)
        {
            ...
            auto query = create_query(items, tds, chl);
            cout << "receiver<request query begin>:    \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
            ...
            cout << "receiver<request query end>:      \t\t" << duration_cast<chrono::milliseconds>(system_clock::now().time_since_epoch()).count() << "ms" << endl;
