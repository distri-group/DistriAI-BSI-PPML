...
template<class T>
Bsi2kTripleGenerator<T>::Bsi2kTripleGenerator(const OTTripleSetup& setup,
        const Names& names, int thread_num, int _nTriples, int nloops,
        MascotParams& machine, mac_key_type mac_key, Player* parentPlayer) :
        NPartyTripleGenerator<T>(setup, names, thread_num, _nTriples, nloops,
                machine, mac_key, parentPlayer)
...
template<class T>
void Bsi2kTripleGenerator<T>::generateTriples()
{
	const int K = T::k;
	const int S = T::s;
	...
	const int TAU = Bsi2kMultiplier<K, S>::TAU;
...
template<class W>
template<class U>
void Bsi2kTripleGenerator<W>::sacrificeZ2k(U& MC, PRNG& G)
{
