...
Bsi2kMultiplier<K, S>::Bsi2kMultiplier(OTTripleGenerator<Bsi2kShare<K, S>>& generator, int thread_num) :
        OTMultiplier<Bsi2kShare<K, S>>
...
template<int K, int S>
Bsi2kMultiplier<K, S>::~Bsi2kMultiplier()
{
    delete mac_vole;
...
template <int K, int S>
void Bsi2kMultiplier<K, S>::init_authenticator(const BitVector& keyBits,
		const vector< array<BitVector, 2> >& senderOutput,
		const vector<BitVector>& receiverOutput) {
...
template <int K, int S>
void Bsi2kMultiplier<K, S>::after_correlation()
{
    this->otCorrelator.reduce_squares(this->generator.nTriplesPerLoop,
            this->c_output);
...
template<int K, int S>
void Bsi2kMultiplier<K, S>::multiplyForInputs(MultJob job)
{
    assert(job.input);
